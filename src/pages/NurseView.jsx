...
// Decodifica el payload del token de Firebase (JWT) para saber quién hizo el
// cambio, sin tener que pasar esa información por separado en cada llamada.
...
// Registro de auditoría: deja constancia de quién cambió qué campo, en qué
// documento y cuándo. No bloquea el guardado si falla (se intenta aparte).
  ...
  return (
    <div style={{ background:"rgba(255,179,71,0.05)", border:"1px dashed rgba(255,179,71,0.3)", borderRadius:12, padding:"16px", marginTop:6 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
       ...
       {med.category !== "domicilio" && <div><label style={labelStyle}>Dilución</label><input required value={draft.diluent} onChange={e => set("diluent",e.target.value)} placeholder="ej: 100 ml SF" style={inputStyle} /></div>}
{med.category !== "domicilio" && <div><label style={labelStyle}>Tiempo (minutos)</label><input type="number" min="1" value={draft.time} onChange={e => set("time",e.target.value)} placeholder="ej: 30" style={inputStyle} /></div>}
      </div>
      <div style={{ display:"flex", gap:8, marginTop:12 }}>
