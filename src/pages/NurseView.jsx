...
async function updateSessionMeds(token, sessionId, meds) {
  ...
  await fetch(
    `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/default/documents/sessions/${sessionId}?updateMask.fieldPaths=meds&updateMask.fieldPaths=authorized`,
    {
      method: "PATCH",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
      body: JSON.stringify({ fields: { meds: toFV(meds), authorized: { booleanValue: false } } }),
    }
  );
}
      ...
      <div style={{ padding:"10px 14px", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontSize:12, color:"#4fc3f7", fontWeight:600 }}>
            Lavado — {wash.solution} · {wash.time} min · {wash.speed ? `${wash.speed} ml/hr` : "—"}
          </div>
          <div style={{ fontSize:11, color:"#555", marginTop:1 }}>
            {ended ? "Lavado completado" : started ? "Lavado en curso" : "Iniciar lavado antes del siguiente medicamento"}
          </div>
        </div>
...
function TimeBtn({ label, time, onRecord, disabled }) {
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", borderRadius:10, background: time ? "rgba(29,158,117,0.08)" : "rgba(255,255,255,0.03)", border:`1px solid ${time ? "rgba(29,158,117,0.25)" : "rgba(255,255,255,0.07)"}` }}>
      ...
      {!time && !disabled && (
        <button onClick={onRecord} style={{ background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.12)", color:"#ddd", borderRadius:8, padding:"7px 14px", fontSize:12, fontWeight:600, cursor:"pointer" }}>Registrar</button>
      )}
...
function SessionCard({ session, token, onRefresh, user }) {
  ...
  const recordEvent = async (key) => {
    try {
      ...
      onRefresh();
    } catch(e) { console.error("Error:", e); alert("Error: " + e.message); }
  };
  ...
  const editMed = async (med) => {
    const newName    = prompt("Nombre del medicamento:", med.name);
    if (!newName) return;
    const newDose    = prompt("Dosis:", med.dose) || med.dose;
    const newDiluent = prompt("Dilución:", med.diluent) || med.diluent;
    const newTime    = parseInt(prompt("Tiempo (min):", med.time) || med.time);
    const updatedMeds = (session.meds || []).map(m =>
      m.id === med.id ? { ...m, name: newName, dose: newDose, diluent: newDiluent, time: newTime } : m
    );
    try {
      const freshToken = await user.getIdToken(true);
      await updateSessionMeds(freshToken, session.id, updatedMeds);
      onRefresh();
  ...
  const deleteMed = async (med) => {
    ...
    const updatedMeds = (session.meds || [])
  ...
  const addMed = async () => {
    ...
    const diluent  = prompt("Dilución:") || "";
    const time     = parseInt(prompt("Tiempo (min):") || "0");
    const category = prompt("Tipo:\n1. premedicacion\n2. inmunoterapia\n3. quimioterapia\n4. adicional\n\nEscribe el nombre:") || "premedicacion";
    const newMed   = { id: Date.now(), order: (session.meds || []).length + 1, name, dose, diluent, time, category };
    const updatedMeds = [...(session.meds || []), newMed];
    try {
      const freshToken = await user.getIdToken(true);
      await updateSessionMeds(freshToken, session.id, updatedMeds);
      onRefresh();
  ...
  const canStartMed = (med) => {
    ...
    if (!medEvents[`med_${prev.id}`]?.fin) return false;
    ...
    <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderLeft:`3px solid ${statusColor}`, borderRadius:16, overflow:"hidden", marginBottom:12 }}>
      ...
      {open && (
        <div style={{ padding:"16px 20px", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
              ...
              return (
                <div key={med.id}>
                  ...
                  <div style={{ borderRadius:11, overflow:"hidden", border:`1px solid ${ended ? "rgba(79,195,247,0.2)" : started ? "rgba(29,158,117,0.2)" : "rgba(255,255,255,0.07)"}`, borderLeft:`3px solid ${color}`, background:"rgba(255,255,255,0.02)" }}>
                    ...
                    {/* Botones editar/eliminar — solo si no ha iniciado */}
                    {!started && session.authorized && (
                      <div style={{ padding:"0 14px 10px", display:"flex", gap:6 }}>
                        <button onClick={() => editMed(med)} style={{ flex:1, padding:"6px", borderRadius:7, fontSize:11, cursor:"pointer", background:"rgba(255,179,71,0.1)", border:"1px solid rgba(255,179,71,0.25)", color:"#ffb347" }}>✏️ Editar</button>
                        <button onClick={() => deleteMed(med)} style={{ flex:1, padding:"6px", borderRadius:7, fontSize:11, cursor:"pointer", background:"rgba(255,107,107,0.1)", border:"1px solid rgba(255,107,107,0.25)", color:"#ff6b6b" }}>🗑 Eliminar</button>
                      </div>
                    )}
                  </div>

                  {/* Lavado después del medicamento */}
          ...
          {/* Agregar medicamento */}
          {session.authorized && (
            <button onClick={addMed} style={{ width:"100%", padding:"9px", borderRadius:9, fontSize:12, fontWeight:600, cursor:"pointer", background:"rgba(0,212,170,0.08)", border:"1px dashed rgba(0,212,170,0.3)", color:"#00d4aa", marginTop:10 }}>
              + Agregar medicamento
            </button>
          )}
